...
def sfe_feature(nye, velocity, csfe, best_match=[0,17], average_version='mean'):
    ...
    for n_segment in range(velocity.shape[0]):
        for n_dis in range(velocity.shape[2]):
            local_pos_velocity = velocity[n_segment,:,n_dis]
            indexes = [value for value in range(n_segment-3,n_segment+4)]
...
def generate_screw_datapoints(flag='p1'):
    # global width
    screw_box = [1045.6448, 308.07604, 704.25007]

    data_dir = 'data/sum-screw-p1-p2_10112023/'
    if flag=='p1':
        csfe_path = data_dir + 'csfe/sfe_local_nicocr_withxz_screw_p1.mat'
        velocity_path = data_dir + 'velocity/nicocr_random_local_segment_velocity_withxz_p1.mat'
        nye13_path = data_dir + 'core_width_from_NYE/screw_nye13-p1-width.mat'
        nye33_path = data_dir + 'core_width_from_NYE/screw_nye33-p1-width_updated.mat'
    elif flag=='p2':
        csfe_path = data_dir + 'csfe/delta_local_nicocr_withxz_screw_p2.mat'
        velocity_path = data_dir + 'velocity/nicocr_random_local_segment_velocity_withxz_p2.mat'
        nye13_path = data_dir + 'core_width_from_NYE/screw_nye13-p2-width.mat'
        nye33_path = data_dir + 'core_width_from_NYE/screw_nye33-p2_width_updated.mat'
    ...
    for num_id in range(17):
        if num_id==0:
            ...
            # df = construct_feature(df,cutoff=[15*(num_id-0.5),15*(num_id+0.5)])
            df = construct_feature(df,nye=nye13,velocity=velocity,csfe=csfe,
                          gradient_csfe_x=gradient_csfe_x,
                          gradient_csfe_y=gradient_csfe_y,
                          cutoff=[0,15*(num_id)])
    ...
    print(df.shape)
    file_path = 'data/screw_' + flag + '_final.csv'
    df.to_csv(file_path,index=False)

if __name__ == "__main__":
    generate_screw_datapoints('p1')
    generate_screw_datapoints('p2')
