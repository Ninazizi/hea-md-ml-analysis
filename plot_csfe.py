...
def cal_pearson(csfe_path,gap=17):
    ...
    vel_y=[]
    vel_y.append(pearson_list[0])
    for ii in range(1,10):
        vel_y.append(0.5*(pearson_list[ii*2-1]+pearson_list[ii*2]))

    pearson_list = []
    for ii in range(0, 10):
        pearson_list.append(pearsonr(csfe_matrix[:, 0], csfe_matrix[:, ii]).statistic)
        print(pearsonr(csfe_matrix[:, 0], csfe_matrix[:, ii]))
    csfe_y = pearson_list
